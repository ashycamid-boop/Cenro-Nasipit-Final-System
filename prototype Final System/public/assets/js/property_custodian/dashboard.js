...
async function fetchAndPopulateCounts() {
  try {
    ...
    const counts = { available: 0, assigned: 0, returned: 0, maintenance: 0, damaged: 0, out_of_service: 0 };
    ...
    if (window.dashboardCharts && window.dashboardCharts.equipmentStatusChart) {
      ...
      chart.data.datasets[0].data = [counts.available, counts.assigned, counts.returned, counts.maintenance, counts.damaged, counts.out_of_service];
      chart.update();
    }
...
function initializeDashboardCharts() {
  ...
  if (ctx2 && typeof Chart !== 'undefined') {
    const labels = ['Available', 'Assigned', 'Returned', 'Under Maintenance', 'Damaged', 'Out of Service'];
    const data = [0, 0, 0, 0, 0, 0];
    const statusChart = new Chart(ctx2, {
      type: 'doughnut',
      data: { labels: labels, datasets: [{ data: data, backgroundColor: ['#0d6efd', '#198754', '#0dcaf0', '#ffc107', '#dc3545', '#6c757d'], borderWidth: 3, borderColor: '#ffffff', hoverOffset: 6 }] },
